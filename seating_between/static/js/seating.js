    ...
    switch (msg.type) {
        ...
        case 'response.function_call_arguments.done':
            if (msg.name === 'show_seat_map') {
                ...
                // Round 1: bot still needs to ask the preference question after showing the map
                // Rounds 2+: map is shown, just wait for participant — no response needed
                if (OTREE_ROUND === 1) {
                    dataChannel.send(JSON.stringify({ type: 'response.create' }));
                }
        ...
        case 'response.done':
            ...
            } else if (seatMapShownAt === null && OTREE_ROUND > 1) {
